...
function CreatePost() {
  ...
  const handleSubmit = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    disPatch(addPost(formData))
      ...
      .catch((error: any) => {
        ...
        const errorData: FormError = error.error
  ...
  const handleUpdatePost = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    disPatch(updatePost(formData))
      ...
      .catch((err: any) => {
        const errorData: FormError = err.error
        setFormError(errorData)
  ...
  return (
    <form>
      ...
      <div className='mb-6'>
        <label
          htmlFor='publishDate'
          className={`mb-2 block text-sm font-medium dark:text-gray-300 ${formError?.publishDate ? 'text-gray-900' : 'text-gray-900'}`}
        >
          Publish Date
        </label>
        <input
